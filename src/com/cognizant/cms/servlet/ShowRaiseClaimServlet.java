...
/**
 * Servlet implementation class ShowRaiseClaimServlet
 */
@WebServlet(name = "ShowRaiseClaim", urlPatterns = { "/ShowRaiseClaim" })
@MultipartConfig(maxFileSize = 99999999) 
public class ShowRaiseClaimServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ShowRaiseClaimServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			PreparedStatement ps=conn.prepareStatement("insert into claim values(?,?,?,?,?,?,?,?,?,?)");
			ps.setString(1,cid);
			ps.setString(2, id);
